    ...
    return (
        <div className='pt-[132px] relative'>
            <div className='max-w-[1104px] mx-auto px-4'>
                <Description text='Problem we solve.' myClass='text-center !leading-[30px] max-sm:!text-2xl max-sm:!leading-[26px] !font-light !text-3xl pb-[7px] max-sm:pb-[5px]' />
                    ...
                    {MARKETING_LIST.map((obj, i) => (
                        <div key={i} className={`w-1/2 max-lg:w-full max-lg:pb-5  ${i === 0 ? "lg:pr-8" : i === 1 ? "lg:pl-8 lg:pb-16" : i === 2 ? "lg:pr-8" : "lg:pl-8"}`}>
                            <div className='border-2 bg-white border-babyGreen rounded-[25px] pt-[65px] pr-[71px] pb-[54px] sm:h-[506px] flex flex-col justify-between max-lg:p-8 max-w-max max-lg:mx-auto'>
                                ...
                                <a href="#learn" className='flex items-center font-maisonDemi text-[15px] leading-5 gap-2 group hover:!text-lemon transition-all duration-300 max-w-max lg:pl-[106px] sm:pl-20 max-sm:pt-8'>{obj.myLink} <LearnMoreArrow myClass='group-hover:fill-lemon transition-all duration-300' /></a>
                            </div>
                        </div>
                    ))}
                </div>
